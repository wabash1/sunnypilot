...
class CarController:
  def __init__(self, dbc_name, CP, VM):
    ...
    self.acc_type_set = False
    self.acc_type_count = 0
  ...
  def update(self, CC, CS, ext_bus):
    ...
    if not CC.cruiseControl.resume and CS.out.cruiseState.enabled and not CS.out.gasPressed and not self.CP.pcmCruiseSpeed:
      if self.frame > self.graMsgStartFramePrev:
  ...
  def type_0(self):
    ...
    if not self.acc_type_set:
      self.acc_type_count += 1
      if self.acc_type_count >= 2:
        if abs(self.v_set_dis - self.v_set_dis_prev) in [5, 10]:
          self.acc_type = 1
          self.param_s.put("VwAccType", str(self.acc_type))
          self.acc_type_set = True
      else:
